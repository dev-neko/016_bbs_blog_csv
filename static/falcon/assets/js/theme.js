...
var chartRadar = function chartRadar() {
	...
	var getOptions = function getOptions() {
		return {
			type: 'radar',
			data: {
				labels: ['aaaEnglish', 'Maths', 'Physics', 'Chemistry', 'Biology', 'History'],
				datasets: [{
					label: 'Student A',
...
var chartRadar = function chartRadar() {
	...
	//Chart.defaults.global.defaultFontSize = '24'; //何も変わらない
	//Chart.defaults.font.size = 10; //凡例だけ変わった
	//Chart.defaults.global.defaultFontColor = 'red'; //チャートが消えた

	var getOptions = function getOptions() {
		return {
			type: 'radar',
			data: {
				labels: [['A 事業戦略','78.6点'],['B 経営手腕','71.3点'],['C 職場環境','68.1点'],['D 仕事の意義','73.2点'],['E 教育成長','65.6点'],['F 給与の処遇','71.1点'],['G 生活しやすさ','67.8点']],
				datasets: [{
					label: '全国平均',
					backgroundColor: utils.rgbaColor(utils.getColor('warning'), 0.5),
					data: [60, 60, 60, 60, 60, 60, 60],
					borderWidth: 1
				}, {
					label: 'ABCD株式会社',
					backgroundColor: utils.rgbaColor(utils.getColor('primary'), 0.5),
					data: [78.6, 71.3, 68.1, 73.2, 65.6, 71.1, 67.8],
					borderWidth: 1
				}]
			},
			options: {
				...
				scales: {
					r: {
						pointLabels: {
							font: {
								size: 12, //https://stackoverflow.com/questions/67643949/chartjs-v3-radar-chart-label-font-size
							},
						},
						grid: {
							color: utils.rgbaColor(utils.getGrays().black, 0.1),
						...
						ticks: {
							...
							stepSize: 100, // 目盛り間隔、100にすると一番外側だけになる
						},
					},
				},
			},
